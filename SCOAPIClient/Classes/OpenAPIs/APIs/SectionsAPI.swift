...
open class SectionsAPI {
    ...
    /**
     ...
     - parameter id: (path) A unique integer value identifying this section. 
     - parameter apiResponseQueue: The queue on which api response is dispatched.
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func sectionsRead(id: Int, apiResponseQueue: DispatchQueue = SCOAPIClientAPI.apiResponseQueue, completion: @escaping ((_ data: Section?,_ error: Error?) -> Void)) {
        sectionsReadWithRequestBuilder(id: id).execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                completion(response.body, nil)
            case let .failure(error):
     ...
     */
    open class func sectionsReadWithRequestBuilder(id: Int) -> RequestBuilder<Section> {
        ...
        let requestBuilder: RequestBuilder<Section>.Type = SCOAPIClientAPI.requestBuilderFactory.getBuilder()
